...
const ChatFAB = ({ onBuy }: { onBuy: (product: string, price: number) => void }) => {
  const [isVisible, setIsVisible] = useState(false);
  const [isOpen, setIsOpen] = useState(false);

  useEffect(() => {
    const handleScroll = () => {
      // Show after scrolling down ~400px (2nd-3rd screen usually)
  ...
  const chatOptions = [
    ...
    { 
      ...
      icon: (
        <svg viewBox="0 0 24 24" fill="white" className="w-5 h-5 ml-[-2px]">
          <path d="M12 2L1 11l1.5 1.5L12 6.5l9.5 6L23 11 12 2z" opacity=".3"/>
    ...
    { 
      ...
      bg: 'bg-[#7360f2]' // Brand Viber color
    },
    { 
      name: 'Order', 
      icon: (
      ...
      onClick: () => {
        onBuy('Золота середина (5 стіків)', 895);
        setIsOpen(false);
      },
      bg: 'bg-accent'
    }
  ...
  return (
    <div className="fixed bottom-8 right-6 z-[2000] flex flex-col items-center gap-4">
      {/* Menu Stack */}
      <div className={`flex flex-col gap-3 transition-all duration-500 ease-out ${isOpen ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-10 scale-0 pointer-events-none'}`}>
        {chatOptions.map((opt, i) => (
          <div key={i} className="flex items-center group">
            <span className="mr-3 px-3 py-1.5 rounded-lg bg-black/80 text-white text-xs font-semibold opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap border border-white/10">
              {opt.name}
            </span>
            {opt.href ? (
              <a 
                href={opt.href} 
                className={`w-12 h-12 rounded-full flex items-center justify-center shadow-xl backdrop-blur-md border border-white/20 hover:scale-110 transition-transform ${opt.bg}`}
              >
                {opt.icon}
              </a>
            ) : (
      ...
      <button 
        ...
        className={`w-14 h-14 rounded-full flex items-center justify-center shadow-[0_8px_32px_rgba(0,0,0,0.5)] backdrop-blur-xl border border-white/20 transition-all duration-300 relative group overflow-hidden ${isOpen ? 'bg-white/10' : 'bg-white/5'}`}
